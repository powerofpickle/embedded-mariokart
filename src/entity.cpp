...
Sprite *shell_sprites[] = {&green_shell_sprite};
...
GreenShell::GreenShell() { initialized = false; }
...
PlayerSprite toad_sprites = {&toad_north_sprite, &toad_east_sprite,
                             &toad_sprite,       &toad_west_sprite,
                             &toad_left_sprite,  &toad_right_sprite};
PlayerSprite bowser_sprites = {&bowser_north_sprite, &bowser_east_sprite,
                               &bowser_sprite,       &bowser_west_sprite,
                               &bowser_left_sprite,  &bowser_right_sprite};
PlayerSprite bullet_bill_sprites = {
    &bullet_bill_north_sprite, &bullet_bill_east_sprite,
    &bullet_bill_sprite,       &bullet_bill_west_sprite,
    &bullet_bill_left_sprite,  &bullet_bill_right_sprite};
...
void LocalPlayer::update() {
  ...
  if (lap != map->player2.lap) {
    ...
    int8_t *next_node =
        map->progress_nodes + ((node_index + 1) % map->num_nodes) * 2;
...
void LocalPlayer::draw() {
  ...
  PlayerSprite *sprites =
      (bullet_bill == 0) ? this->sprites : &bullet_bill_sprites;
  Sprite *s;
  s = Input::steering_angle > 0.01
          ? sprites->left
          : (Input::steering_angle < -0.01 ? sprites->right : sprites->south);
