...
Sprite::Sprite(uint16_t *pixels, uint8_t width, uint8_t height, int8_t offx,
               int8_t offy) {
  this->pixels = pixels;
  this->width = width;
  this->height = height;
...
void Sprite::draw(int x, int y, float scale) {
  ...
  if (!((x + ox) >= 0 && (x + ox + scaled_w) < WIDTH &&
        (y + oy + scaled_h) < HEIGHT / 2 && (y + oy) >= 0))
    return;
...
void Sprite::draw(int x, int y, float scale){
  ...
  if(!((x + ox) >= 0 && (x + ox + scaled_w) < WIDTH && (y + oy + scaled_w) <
HEIGHT/2 && (y + oy) >= 0)) return;
