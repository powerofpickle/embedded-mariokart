...
void Input::update() {
  ...
  Input::acceleration =
      (glfwGetKey(screen.window, FORWARD_B) == GLFW_PRESS) ? 0.005f : -0.005f;

  float left_steer =
      (glfwGetKey(screen.window, LEFT_B) == GLFW_PRESS) ? 0.25f : 0;
  float right_steer =
      (glfwGetKey(screen.window, RIGHT_B) == GLFW_PRESS) ? -0.25f : 0;
